...
FILES[".github/workflows/compliance-license.yml"] = """\
...
FILES[".github/workflows/compliance-security.yml"] = """\
...
# ── 3. check-license.sh ───────────────────────────────────────
FILES[".github/scripts/compliance/check-license.sh"] = """\
#!/usr/bin/env bash
...
# ── 4. check-security.sh ─────────────────────────────────────
FILES[".github/scripts/compliance/check-security.sh"] = """\
#!/usr/bin/env bash
...
# ── 5. post-comment.js ────────────────────────────────────────
FILES[".github/scripts/compliance/post-comment.js"] = """\
...
async function main() {
  ...
  if (result.status === 200 || result.status === 201) {
    ...
    console.error("❌ Failed:", result.body); process.exit(1);
  }
}
...
def main():
    print(f"\n{BOLD}{'═' * 48}{RESET}")
    print(f"{BOLD}🚀 Compliance Setup{RESET}")
    ...
    for filepath, content in FILES.items():
        ...
        # ถ้าไฟล์มีอยู่แล้ว — ถามก่อน overwrite
        if os.path.exists(filepath):
            ans = input(f"{YELLOW}⚠️  {filepath} already exists. Overwrite? [y/N]: {RESET}").strip().lower()
            if ans != "y":
                skipped.append(filepath)
                warn(f"Skipped : {filepath}")
                continue
        ...
        try:
            ...
            # ให้สิทธิ์ execute สำหรับ shell scripts
            if filepath in SHELL_SCRIPTS:
            ...
            err(f"Failed  : {filepath} → {e}")

    # ── สรุปผล ────────────────────────────────────────────────
    print(f"\n{BOLD}{'═' * 48}{RESET}")
    print(f"{BOLD}📊 Summary{RESET}")
    print(f"{'═' * 48}")
    print(f"  {GREEN}Created : {len(created)}{RESET}")
  ...
  .github/
  ├── workflows/
  │   ├── compliance-license.yml
  │   └── compliance-security.yml
  └── scripts/
      └── compliance/
          ├── check-license.sh
          ├── check-security.sh
    ...
    print(f"""\
  {CYAN}git update-index --chmod=+x .github/scripts/compliance/check-license.sh
  git update-index --chmod=+x .github/scripts/compliance/check-security.sh
  git add .github/
  git commit -m "chore: add compliance workflows and scripts"
  git push{RESET}""")
